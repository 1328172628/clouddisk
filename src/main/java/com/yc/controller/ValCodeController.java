...
public class ValCodeController {
    ...
    public void VerifyCode(HttpSession session, HttpServletRequest req, HttpServletResponse resp)
        ...
        //TODO: 使用分布式的话  不能这样用session   可以使用共享session  这里采用redis
        //  获取用户ip 以ip+"code"为键 保证高并发 场景下 不会发生 验证码冲突 互相添加缓存导致别人验证码失效
        String userIp = GetUserInfo.getIPAddress(req);
        Jedis jedis = RedisBloom.getRedis();
        jedis.setex(userIp+"-code", 60,randomCode.toString());
